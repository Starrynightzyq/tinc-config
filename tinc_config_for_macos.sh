...
OLD_IFS="$IFS"
IFS="/"
iparray=($local_ipv4_addr)
IFS="$OLD_IFS"
local_ipv4_addr_nomask=${iparray[0]}
...
sudo mkdir -p /usr/local/etc/tinc/$net_name/hosts
sudo chmod -R 755 /usr/local/etc/tinc/$net_name
cd /usr/local/etc/tinc/$net_name/
...
sudo echo "Device = /dev/tap0" | sudo tee -a  tinc.conf
sudo echo "PrivateKeyFile=/usr/local/etc/tinc/$net_name/rsa_key.priv" | sudo tee -a  tinc.conf

############### tinc-up #################
sudo touch tinc-up
sudo echo "#!/bin/sh" | sudo tee -a  tinc-up
sudo echo "ifconfig $INTERFACE $local_ipv4_addr_nomask mtu 1500 netmask 255.255.255.0" | sudo tee -a  tinc-up
...
echo "done"
echo "start command: sudo tincd -n netname -D -d3"
